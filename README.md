...
- 页签支持鼠标拖拽排序，也可聚焦页签后使用方向键调整位置；顺序会随会话自动保存。
...
向编辑器粘贴代码时，会根据粘贴片段的高信号语法自动识别 JSON、HTML/Vue、SQL、YAML、JavaScript/TypeScript、Python、Go、Rust、C/C++、Java、C#、Swift、Shell、CSS 和 Markdown，并只格式化本次粘贴范围。识别不明确时沿用当前页签语言；没有可用格式化器时保留原文，不会阻断粘贴。
...
更新检查只比较正式语义化版本号；没有匹配当前系统的安装包时会打开 Release 页面。macOS 包未使用 Apple Developer ID 公证，Windows 安装包未签名；对应系统首次运行可能出现安全提示。
